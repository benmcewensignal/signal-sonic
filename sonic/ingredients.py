"""Monthly ingredient series per scene.

The app holds each scene's displacement month by month but only current values for the
named ingredients, so a question like "what has vocal presence done this year" can be
answered as a number but not drawn as a line. This publishes the monthly series for
vocal presence, drums, swing, bass, tempo, and the near-instrumental share — the last
being where the vocal finding actually lives.

  python -m sonic.ingredients --db sonic.db --out data/ingredient-series.json
"""
