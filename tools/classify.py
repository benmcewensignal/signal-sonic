...
te = np.array([i for i in range(len(X)) if A[i] in hold]); tr = np.array([i for i in range(len(X)) if A[i] not in hold])
...
acc = float(np.mean(pred == S[te])); per = {t: round(float(np.mean(pred[S[te] == t] == t)), 3) for t in tags}
...
months = sorted(set(W)); cut = months[len(months) // 2]; early = W < cut; late = ~early
...
out = {"records": len(X), "tags": tags, "artists_held_out": len(hold), "accuracy_36": acc, "per_tag": per, "confused_pairs": pairs[:40],
...
worst = sorted(per.items(), key=lambda kv: kv[1])[:6]; best = sorted(per.items(), key=lambda kv: -kv[1])[:5]
